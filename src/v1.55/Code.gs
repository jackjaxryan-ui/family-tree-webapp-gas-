...
function getFamiliesIndex() {
  ...
  return ctx.families
    ...
    .sort((a, b) => {
      // 🔥 Sort by pasangan (gabungan nama biar konsisten)
      const nameA = (a.person1Name + ' ' + a.person2Name).toLowerCase();
      const nameB = (b.person1Name + ' ' + b.person2Name).toLowerCase();
...
function sortChildLinks_(links, ctx) {
  return (links || []).slice().sort((a, b) => {
    const aPerson = ctx.personById[a.ChildID] || {};
    const bPerson = ctx.personById[b.ChildID] || {};

    const aYear = Number(aPerson.BirthYear || 0);
    const bYear = Number(bPerson.BirthYear || 0);

    // 1. Prioritas BirthYear
    ...
    // 3. Final fallback → ID
    return String(a.ChildID || '').localeCompare(String(b.ChildID || ''), 'id');
  });
}
...
function getFamilyChildrenNodesFromCtx_(ctx, familyId) {
  const links = sortChildLinks_(ctx.childrenByFamilyId[familyId] || [], ctx);
...
function getFamilyTreeView(familyId) {
  ...
  const childLinks = sortChildLinks_(ctx.childrenByFamilyId[familyId] || [], ctx);
